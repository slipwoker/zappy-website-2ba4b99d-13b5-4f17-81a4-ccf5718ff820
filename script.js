  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      contactForm.querySelectorAll('input, textarea, select').forEach(function(field) {
        ...
        // Phone validation (if field has tel type or name contains phone)
        else if ((field.type === 'tel' || fieldName.toLowerCase().includes('phone')) && fieldValue) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "טל רבי משרד עורכי דין משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '2ba4b99d-13b5-4f17-81a4-ccf5718ff820',
                    name: data.name || '',
                    email: data.email || '',
